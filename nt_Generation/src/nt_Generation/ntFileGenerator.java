...
public class ntFileGenerator {
	...
	private void write_file(BufferedWriter bw) throws IOException {
		...
		for (Variable variable : this.automat.all_variables) {
		...
		this.write_class("Global", bw);
		for (Supernode supernode : this.automat.all_supernodes) {
			this.write_class(supernode.name, bw);
		}
		// create NamedIndividuals
		this.write_instance("global", "", bw);
		for (Supernode supernode: this.automat.all_supernodes) {
			this.write_instance(supernode.name, supernode.parent_name, bw);
			}
	}
	...
	public ntFileGenerator(SceneMakerAutomaton automat, String uri) {
