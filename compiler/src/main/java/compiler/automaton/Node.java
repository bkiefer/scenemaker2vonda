...
public class Node {
  ...
  /**
   * The id (unique name) of the {@code Node}.
   */
  private String id;
  /**
  ...
  */
  ...
  public Node() {
	...
	public String getNodeCode() {
	  ...
	  String outString = RudiFileGenerator.formattedRuleLabel(this.getName(), 0, 0, 0);
	  outString += RudiFileGenerator.formattedIfOpening(this.parent.getName() + ".simple_children.contains(\"" + this.getName() + "\")", 1, 1, 2);

	  outString += this.getInterruptiveEdgesCode(this.getInterruptiveEdges(), 2);

	  if (!this.code.isEmpty()) {

		  outString += RudiFileGenerator.formattedIfOpening(this.parent.getName() + ".imminent_simple_children.contains(\"" + this.getName() + "\")", 0, 2, 2);
	  ...
	  outString += RudiFileGenerator.formattedLine(this.parent.getName() + ".imminent_simple_children -= \"" + this.getName() + "\"", 1, 2, 2);
	  ...
	  if(this.getForkEdges().isEmpty() == false) {
		  outString += RudiFileGenerator.formattedLine(this.parent.getName() + ".simple_children -= \"" + this.getName() + "\"", 1, 2, 1);
	  }

	  if(this.processCanDieHere()) {
		  outString +=  RudiFileGenerator.formattedLine("check_out_transition(\"" + this.getName() + "\", \"" + this.parent.getName() + "_out\", " + this.parent.getName() + ", " + this.parent.getName() + ")", 1, 2, 1);
	  }
